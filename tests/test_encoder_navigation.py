...
def test_shelf_backout_waits_for_recorded_ultrasonic_distance():
    serial, clock = FakeSerial(), FakeClock()
    ...
    assert controller.plan["shelf_entry_ultrasonic_cm"] == 42.0
    assert controller.plan["return"][0]["target_ultrasonic_cm"] == 42.0
    assert controller.get_status()["shelf_entry_ultrasonic_cm"] == 42.0

    controller.phase = "RETURNING"
    controller.step_index = 0
    controller._start_current_step()
    target_ticks = controller.plan["return"][0]["target_ticks"]
    serial.encoders = {"left": target_ticks, "right": target_ticks}
    serial.ultrasonic["center"] = 10

    controller.step()

    assert controller.get_status()["current_action"] == "BACKWARD"
    assert controller.get_status()["return_ultrasonic_active"] is True

    serial.ultrasonic["center"] = 42
    controller.step()
    assert controller.get_status()["current_action"] == "BACKWARD"
    assert controller.get_status()["return_ultrasonic_match_frames"] == 1
    assert serial.commands[-1] == "STOP"

    controller.step()

    assert controller.get_status()["current_action"] == "TURN_RIGHT"
    assert serial.turn_requests[-1] == ("right", 90)


def test_shelf_backout_rejects_no_echo_as_a_distance_match():
    serial, clock = FakeSerial(), FakeClock()
    controller = GridController(serial, clock=clock, turn_source="imu")
    plan = build_grid_route(
        "1A",
    ...
    controller.request_grid_mission(plan)
    controller.plan["shelf_entry_ultrasonic_cm"] = 42.0
    controller.plan["return"][0]["target_ultrasonic_cm"] = 42.0
    controller.phase = "RETURNING"
    controller.step_index = 0
    controller._start_current_step()
    serial.ultrasonic["center"] = 10
    ...
    serial.ultrasonic["center"] = 999
    serial.encoders = {"left": 6, "right": 6}
    controller.step()

    assert controller.get_status()["current_action"] == "BACKWARD"
    assert controller.get_status()["return_ultrasonic_match_frames"] == 0

    serial.encoders = {"left": 7, "right": 7}
    controller.step()

    assert controller.get_state() == GridState.STOPPED.value
    assert (
        controller.get_status()["reason"]
        == "return_ultrasonic_target_not_reached"
    )
