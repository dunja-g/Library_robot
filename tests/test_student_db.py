...
def test_same_book_cannot_be_borrowed_twice(monkeypatch, tmp_path):
    seed_students(monkeypatch, tmp_path)
    assert student_db.borrow_book("S001", "BK001") == {"ok": True}

    result = student_db.borrow_book("S999", "BK001")

    assert result == {"ok": False, "reason": "book_unavailable"}
