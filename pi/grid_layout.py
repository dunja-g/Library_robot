...
def outward_turn_for(side: str) -> str:
    return "TURN_LEFT" if side == "A" else "TURN_RIGHT"


def reverse_turn_for(side: str) -> str:
    return "TURN_RIGHT" if side == "A" else "TURN_LEFT"
...
def build_grid_route(
    ...
    row = int(box_id[0])
    side = box_id[1]
    outward_turn = outward_turn_for(side)
    reverse_turn = reverse_turn_for(side)
