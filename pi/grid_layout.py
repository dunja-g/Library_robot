...
class EncoderCalibration:
    ...
    def distance_ticks(self, distance_cm: float) -> int:
        if self.ticks_per_cm is None:
            raise ValueError("Encoder ticks_per_cm is not configured")
        return max(1, round(distance_cm * self.ticks_per_cm))
...
def build_grid_route(
    ...
    missing = geometry.missing_fields + calibration.missing_fields
    if missing:
    ...
    outbound = [
        ...
        {
            "action": outward_turn,
            "target_ticks": int(calibration.turn_90_ticks),
    ...
    return_route = [
        {
            "action": "UTURN",
            "target_ticks": int(calibration.turn_180_ticks),
